...
const TeamGamePage = () => {
    ...
    const {
        ...
        selectedTeam,
        teamName,
        displayName: initialDisplayName // Passed from Create/Join
    } = location.state || {};

    // Identity Helper (matching lobby logic)
    const myName = initialDisplayName || currentPlayer?.name || member?.name;
    const myUid = (!member?.email || member.email === 'guest@sortonym.com')
        ? `guest_${myName?.toLowerCase().trim().replace(/[^a-z0-9]/g, '_')}`
        : member.email;
    const isMe = (pid) => pid === myUid;
    ...
    const handleSubmit = async () => {
        ...
        const submissionData = {
            ...
            displayName: myName // Use the established myName helper for consistent ID matching
        };

        try {
            const res = await submitGame(submissionData);
/// compulsary hanl=dle round_level on the table 
            const roundScore = {
                round: roundCount + 1,
                score: res.score || 0,
    ...
    if (gameState === 'waiting_for_others') {
        return (
            <div className="game-page-v2 loading-state" style={{ backgroundColor: '#f8fafc' }}>
                <div className="loading-container" style={{ textAlign: 'center', padding: '2rem' }}>
                    ...
                    <h2 style={{ fontSize: '24px', fontWeight: '800', color: '#1e293b' }}>Rounds Complete!</h2>
                    <p style={{ color: '#64748b', fontSize: '16px' }}>Waiting for other players to finish their 5 rounds...</p>
                    <div style={{ marginTop: '20px', padding: '12px', background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', display: 'inline-block' }}>
                        <i className="bi bi-people-fill" style={{ marginRight: '8px', color: '#3b82f6' }}></i>
                        <span style={{ fontWeight: '600' }}>Game syncing...</span>
                    </div>
                </div>
            </div>
        );
